...
kappa_el = 1.1; %S/m
tmax = 2.61; % time for forward or reverse sweep
...
t = linspace(0,2*tmax*3600/abs(iapp),100000);
...
% Numerically integrate to find phi_av and capacity at each time
phi_av = []; capacity = []; dt = t(2)-t(1); cpv = 0;
V_cell = []; V_cell_SPM = [];
%
for i=1:1:length(t)
    phi_av(i)=(ll/L1)*trapz(x(1:100),phi_el(i,:)); 

    if t(i)<=tmax*3600/abs(iapp)
    iappv = iapp;
    elseif t(i)>tmax*3600/abs(iapp) && t(i)<=(tmax+0.01)*3600/abs(iapp)
    iappv = iapp*(1-(t(i)-tmax*3600/abs(iapp))/(0.005*3600/abs(iapp)));        
    else
    iappv = -iapp; 
    end
    cpv = cpv + iappv*dt./3600*1.0e3;
    capacity = [capacity cpv];

    % eSPM Vcell
    V_cell_v = V0-muf(i)./FF-iappv*(L1/3/kappa_s)+phi_av(i)*R*T/FF+2*R*T/FF*asinh(-iappv/iex/2); 
    V_cell = [V_cell V_cell_v];
    
    % SPM Vcell
    V_cell_SPM_v = V0-muf(i)./FF+2*R*T/FF*asinh(-iappv/iex/2); 
    V_cell_SPM = [V_cell_SPM V_cell_SPM_v];
end
%

figure
plot(capacity,V_cell,'r-')
xlabel('Capacity (mA-hr)','Interpreter','latex');
ylabel('Half cell voltage (V)','Interpreter','latex');
hold on
plot(capacity,V_cell_SPM,'r--')
...
global iapp R T FF Npmax ll tau tmax
%
if t<=tmax*3600/abs(iapp)
iapp_nd = iapp*tau/(ll*Npmax*FF); % iapp_hat in the notes
elseif t>tmax*3600/abs(iapp) && t<=(tmax+0.01)*3600/abs(iapp)
iapp_nd = iapp*tau/(ll*Npmax*FF)*(1-(t-tmax*3600/abs(iapp))/(0.005*3600/abs(iapp)));        
else
iapp_nd = -iapp*tau/(ll*Npmax*FF); 
end
% 
